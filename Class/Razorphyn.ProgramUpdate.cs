...
namespace OrderManager.Class
{
    public class ProgramUpdateFunctions
    {
        ...
        public bool PostUpdateDataManipulation(decimal version)
        {
            ...
            if (version == 5)
            {
                operationSuccess = Updates.Post.Update_5(version);
            }
            else if (version == 6)
            {
                operationSuccess = Updates.Post.Update_6();
            }
            else if (version == 10)
            {
                operationSuccess = Updates.Post.Update_10(version);
            }
    ...
    internal class Updates
    {
        internal class Post
        {
            internal static bool Update_5(decimal version)
            {
                ...
                {
                    Outlook.Items restrictedItems = CalendarInteropOutlook.CalendarGetItems(personalCalendar, Convert.ToDateTime(startDate).AddDays(-1), Convert.ToDateTime(startDate).AddDays(+1));
                ...
                using (var f = new ManagerOrdini.Forms.Update.U6(DictClienti))
                {
                    ...
                    if (result == DialogResult.OK)
                    {
                        ...
                        foreach (Cliente cliente in ListClienti)
                        {
                            ...
                            {
                                commandText += @"INSERT INTO " + ProgramParameters.schemadb + "[clienti_elenco_temp] (id, nome) VALUES (@idcl,@nomecl);" + Environment.NewLine;
                            }

                            commandText += @"INSERT INTO " + ProgramParameters.schemadb + "[clienti_sedi] (ID_cliente, ID_cliente_old, stato, provincia, citta) VALUES (@idcl, @idclold, @clstato, @clprovincia, @citta);" + Environment.NewLine;
            ...
            internal static bool Update_10(decimal version)
            {
                bool operationSuccess = true;
                        ...
                        try
                        {
                            ...
                            if (id_offerta > 0)
                            {
                                ...
                                if (id_ordine > 0)
                                {
                                    ...
                                    using (var f = new ManagerOrdini.Forms.Update.U10_Ordine(ItemsOrder, id_ordine))
                                    {
                                        f.ShowDialog();
                                    }
                                }
                ...
                //Remove duplicate codes in orders
                found = false;
                do
                {

                    commandText = @"SELECT 
                                        OP.[ID_ordine] AS ID_ordine,
                                        PR.[Codice] AS Codice
                                  FROM " + ProgramParameters.schemadb + @"[ordine_pezzi] AS  OP
                                  LEFT JOIN " + ProgramParameters.schemadb + @"[pezzi_ricambi] AS PR
  	                                ON PR.Id = OP.ID_Ricambio
                                  WHERE substr(PR.[Codice],1,1) != 'D' 
                                  GROUP BY OP.[ID_ordine],PR.[Codice],OP.[ETA] HAVING COUNT(OP.[ID_ordine]) > 1;";
                    ...
                    using (SQLiteCommand cmd = new(commandText, temp_connection))
                    {
                        try
                        {
                            ...
                            using (SQLiteDataReader reader = cmd.ExecuteReader())
                            {
                                while (reader.Read())
                                {
                                    id_ordine = Convert.ToInt64(reader["ID_ordine"]);
                                    list.Add(Convert.ToString(reader["Codice"]));
                                }
                            }

                            if (id_ordine > 0)
                            {
                                List<U10_Ricambio> ItemsOrder = U10_Ordine_Ricambio.Ordine_GetItemCollection(id_ordine, list, temp_connection);
                            ...
                            while (answer != DialogResult.Yes)
                            {
                                ...
                                foreach (KeyValuePair<string, Dictionary<long, string>> group in items)
                                {
                                    ...
                                    foreach (KeyValuePair<long, string> item in group.Value)
                                    {
                                        ...
                                        {
                                            replace.Add(item.Key, id.LongValue);
