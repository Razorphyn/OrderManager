...
using static OrderManager.Class.CalendarManager;
...
namespace OrderManager
{
    internal partial class Main : Form
    {
        ...
        internal Main()
        {
            ...
            this.Text = Assembly.GetExecutingAssembly().GetName().Name + " - v" + FileVersionInfo.GetVersionInfo(Application.ExecutablePath).FileVersion;
            ...
            {
                Populate.Populate_combobox_dummy(ctrl);
            }

            Populate.Populate_combobox_dummy(ComboBoxOrdSede);
            Populate.Populate_combobox_dummy(FieldOrdOggSede);
            Populate.Populate_combobox_dummy(ComboBoxOrdCliente);
            Populate.Populate_combobox_dummy(ComboBoxOrdOfferta);
            Populate.Populate_combobox_dummy(ComboSelOrd);
            Populate.Populate_combobox_dummy(FieldOrdStato);
            Populate.Populate_combobox_dummy(ComboSelOrdSede);

            Populate.Populate_combobox_dummy(AddOffCreaPRef);
            Populate.Populate_combobox_dummy(AddOffCreaOggettoMach);
            Populate.Populate_combobox_dummy(ComboBoxOrdContatto);

            Populate.Populate_combobox_dummy(FieldOrdOggMach);
            Populate.Populate_combobox_dummy(FieldOrdOggPezzo);

            Populate.Populate_combobox_dummy(dataGridViewMacchina_Filtro_Cliente);

            Populate.Populate_combobox_FieldOrdSpedGestione(FieldOrdSpedGestione);
            Populate.Populate_combobox_FieldOrdSpedGestione(AddOffCreaSpedizioneGest);
            Populate.Populate_combobox_statoOrdini(new ComboBox[] { FieldOrdStato });
        ...
        private void BtDbBackup_Click(object sender, EventArgs e)
        {
            ...
            string db_name = (ProgramParameters.exeFolderPath + ProgramParameters.db_folder_path + ProgramParameters.db_file_name).ToUpper().ToString();
            ...
            using (FolderBrowserDialog db_backup_path = new())
            {
                ...
                if (OnTopMessage.ShowFolderBrowserDialog(db_backup_path) == DialogResult.OK)
                {
                    ...
                    using (var source = new SQLiteConnection("Data Source=" + ProgramParameters.exeFolderPath + ProgramParameters.db_folder_path + ProgramParameters.db_file_name))
            ...
            using (OpenFileDialog openFileDialog = new())
            {
                ...
                if (!String.IsNullOrEmpty(filePath))
                {
                    ...
                    if (dialogResult == DialogResult.Yes)
                    {
                        ...
                        using (var destination = new SQLiteConnection("Data Source=" + ProgramParameters.exeFolderPath + ProgramParameters.db_folder_path + ProgramParameters.db_file_name))
                        {
                            source.Open();
                            destination.Open();
                            source.BackupDatabase(destination, "main", "main", -1, null, 0);
        ...
        private static void RunSQLiteOptimize(int entry = 150)
        {
        ...
        private void AddDatiCompCliente_SelectedIndexChanged(object sender, System.EventArgs e)
        {
            ...
            if (curItemValue > 0)
            {
                ...
                Populate.Populate_combobox_sedi(new ComboBox[] { AddDatiCompSede }, curItemValue);
            ...
            else
            {
                ...
                Populate.Populate_combobox_dummy(AddDatiCompMachine);
                Populate.Populate_combobox_dummy(AddDatiCompSede);
        ...
        private void ChangeDatiCompCliente_SelectedIndexChanged(object sender, System.EventArgs e)
        {
            ...
            if (idcl > 0)
            {
                ...
                Populate.Populate_combobox_sedi(new ComboBox[] { ChangeDatiCompSede }, idcl);
            ...
            else
            {
                ...
                Populate.Populate_combobox_dummy(ChangeDatiCompMachine);
                Populate.Populate_combobox_dummy(ChangeDatiCompSede);
        ...
        private void ChangeDatiPRefClienti_SelectedIndexChanged(object sender, EventArgs e)
        {
            ...
            if (curItemValue > 0)
            {
                Populate.Populate_combobox_sedi(new ComboBox[] { ChangeDatiPRefSede }, curItemValue);
                ChangeDatiPRefSede.Enabled = true;
            }
            else
            {
                ChangeDatiPRefSede.Enabled = false;
                Populate.Populate_combobox_dummy(ChangeDatiPRefSede);
        ...
        private void AddDatiPRefCliente_SelectedIndexChanged(object sender, EventArgs e)
        {
            ...
            if (curItemValue > 0)
            {
                Populate.Populate_combobox_sedi(new ComboBox[] { AddDatiPRefSede }, curItemValue);
                AddDatiPRefSede.Enabled = true;
            }
            else
            {
                AddDatiPRefSede.Enabled = false;
                Populate.Populate_combobox_dummy(AddDatiPRefSede);
        ...
        private void ChangeDatiMacchinaCliente_SelectedIndexChanged(object sender, EventArgs e)
        {
            ...
            if (curItemValue > 0)
            {
                Populate.Populate_combobox_sedi(new ComboBox[] { ChangeDatiMacchinaSede }, curItemValue);
                ChangeDatiMacchinaSede.Enabled = true;
            }
            else
            {
                ChangeDatiMacchinaSede.Enabled = false;
                Populate.Populate_combobox_dummy(ChangeDatiMacchinaSede);
        ...
        private void AddDatiMacchinaCliente_SelectedIndexChanged(object sender, EventArgs e)
        {
            ...
            if (curItemValue > 0)
            {
                Populate.Populate_combobox_sedi(new ComboBox[] { AddDatiMacchinaSede }, curItemValue);
                AddDatiMacchinaSede.Enabled = true;
            }
            else
            {
                AddDatiMacchinaSede.Enabled = false;
                Populate.Populate_combobox_dummy(AddDatiMacchinaSede);
        ...
        private void AddOffCreaCliente_SelectedIndexChanged(object sender, System.EventArgs e)
        {
            ...
            if (curItemValue > 0)
            {
                Populate.Populate_combobox_sedi(new ComboBox[] { AddOffCreaSede }, curItemValue);
                Populate.Populate_combobox_pref(AddOffCreaPRef, curItemValue);
            ...
            else
            {
                ...
                Populate.Populate_combobox_dummy(AddOffCreaSede);
                Populate.Populate_combobox_dummy(AddOffCreaPRef);
        ...
        private void AddOffCreaSede_SelectedIndexChanged(object sender, System.EventArgs e)
        {
            ...
            if (idsd > 0 && idcl > 0)
            {
                Populate.Populate_combobox_pref(AddOffCreaPRef, idcl, idsd);
        ...
        private void BtCreaOfferta_Click(object sender, EventArgs e)
        {
            ...
            OrderManager.Class.Offerte.Answer esito = OrderManager.Class.Offerte.GestioneOfferte.CreateOffer(dataoffValue.DateValue, numeroOff, idsd, stato, idpref, prezzoSpedizione.DecimalValue, gestSP);
        ...
        private void SelOffCreaCl_SelectedIndexChanged(object sender, EventArgs e)
        {
            ...
            if (idcl > 0)
                Populate.Populate_combobox_sedi(new ComboBox[] { SelOffCreaSede }, idcl);
            else
                Populate.Populate_combobox_dummy(SelOffCreaSede);
        }
        ...
        internal void SelOffCrea_SelectedIndexChanged(object sender, System.EventArgs e)
        {
            ...
            if (curItemValue > 0)
            {
                ...
                using (SQLiteCommand cmd = new(commandText, ProgramParameters.connection))
                {
                    try
                    {
                        ...
                        Populate.Populate_combobox_sedi(new ComboBox[] { AddOffCreaOggettoSede }, idcl);
            ...
            else
            {
                ...
                Populate.Populate_combobox_dummy(AddOffCreaOggettoMach);
                Populate.Populate_combobox_dummy(AddOffCreaOggettoRica);
                Populate.Populate_combobox_dummy(AddOffCreaOggettoSede);
        ...
        private void BtAddRicToOff_Click(object sender, EventArgs e)
        {
            ...
            OrderManager.Class.Offerte.Answer esito = OrderManager.Class.Offerte.GestioneOggetti.AddObjToOffer(idof, idir, prezzoOrV.DecimalValue, prezzoScV.DecimalValue, qtaV.IntValue);
        ...
        private void DataGridViewOffCrea_CellDoubleClick(object sender, DataGridViewCellEventArgs e)
        {
            ...
            if (dgv.SelectedRows.Count == 1)
            {
                foreach (DataGridViewRow row in dgv.SelectedRows)
                {
                    ...
                    DataValidation.ValidationResult answer = OrderManager.Class.Offerte.GetResources.GetContatto(Convert.ToInt64(id));
                    if (!String.IsNullOrEmpty(answer.Error))
                    {
                    ...
                    Populate.Populate_combobox_pref(AddOffCreaPRef, id_cliente, id_sede);
        ...
        private void DataGridViewOffCreaOggetti_CellDoubleClick(object sender, DataGridViewCellEventArgs e)
        {
            ...
            if (dgv.SelectedRows.Count == 1)
            {
                ...
                foreach (DataGridViewRow row in dgv.SelectedRows)
                {
                    ...
                    Populate.Populate_combobox_sedi(new ComboBox[] { AddOffCreaOggettoSede }, id_cliente);
        ...
        private void BtDelChangesOffOgg_Click(object sender, EventArgs e)
        {
            ...
            OrderManager.Class.Offerte.Answer esito = OrderManager.Class.Offerte.GestioneOggetti.DeleteItemFromOffer(idof, (long)idQ.LongValue);
        ...
        private void BtSaveChangesOffOgg_Click(object sender, EventArgs e)
        {
            ...
            OrderManager.Class.Offerte.Answer esito =
                OrderManager.Class.Offerte.GestioneOggetti.UpdateItemFromOffer(idof, (long)idOggToOff, (decimal)prezzoOrV.DecimalValue, (decimal)prezzoScV.DecimalValue, (int)qtaV.IntValue);
                    ...
                    if (!String.IsNullOrEmpty(filePath))
                    {
                        ...
                        using (var document = UglyToad.PdfPig.PdfDocument.Open(filePath))
                        {
                            ...
                            if (wordsCollection.Count() > 0)
                            {
                                ...
                                Offerlang = ImportPDF.Offerte.IdentifyLanguage(words);
                            }

                            if (Offerlang == "")
                            {
                                isPDFImages = (OnTopMessage.Question("Il PDF è costituito da immagini?") == DialogResult.Yes);
                                if (isPDFImages == true)
                                {

                                    Offerlang = ImportPDF.Helper.GetImageLanguage();

                                    ImageSavePath = ProgramParameters.exeFolderPath + System.IO.Path.AltDirectorySeparatorChar + System.IO.Path.GetFileNameWithoutExtension(filePath) + System.IO.Path.AltDirectorySeparatorChar;
                                    Directory.CreateDirectory(ImageSavePath);
                                    OCR.ExtarctImagesFromPDF(filePath, ImageSavePath);
                                    words = OCR.ExtractWordFromImage(ImageSavePath, Offerlang, 1);
                                    words = words.OrderBy(a => a.X).ThenBy(a => a.Y).ToList();
                                }
                                else
                                {
                            ...
                            if (!isPDFImages)
                            {
                                string text = ImportPDF.Helper.ExtractBodyPDF(filePath);
                                lines = text.Split('\n');
                            }
                            else
                                lines = OCR.ExtractLineFromImage(ImageSavePath, Offerlang);

                            offerInfo = ImportPDF.Offerte.ExtractHeader(words, lines, Offerlang, isPDFImages).ToDictionary(kvp => kvp.Key, kvp => kvp.Value);
                        }
                            ...
                            Dictionary<string, string> itemInfo = new()
                        {
                            ...
                            Match code = Regex.Match(currentLine.Replace(", ", ","), patterCode, RegexOptions.IgnoreCase);
                            if (code.Success)
                            {
                                Dictionary<string, Dictionary<string, string>> findStrField = ImportPDF.Helper.GetDictionarySDSS("OffertaItem");
                        ...
                        if (isPDFImages == true && Directory.Exists(ImageSavePath))
        ...
        private void ComboBoxOrdCliente_SelectedIndexChanged(object sender, System.EventArgs e)
        {
            ...
            if (curItemValue > 0)
            {
                Populate.Populate_combobox_sedi(new ComboBox[] { ComboBoxOrdSede }, curItemValue);
        ...
        private void ComboBoxOrdSede_SelectedIndexChanged(object sender, System.EventArgs e)
        {
            ...
            if (idsd > 0)
            {
                Populate.Populate_combobox_ordini_crea_offerta(ComboBoxOrdOfferta, idcl, idsd);
                Populate.Populate_combobox_pref(ComboBoxOrdContatto, idcl, idsd);
            ...
            else
            {
                ...
                Populate.Populate_combobox_pref(ComboBoxOrdContatto, idcl);
                ComboBoxOrdContatto.Enabled = true;
        ...
        private void LoadOrdiniTable(int page = 1)
        {
            ...
            string numOrdineFilter = DataGridViewOrd_Filter_NumOrdine.Text.Trim();
        ...
        internal void ComboSelOrd_SelectedIndexChanged(object sender, System.EventArgs e)
        {
            ...
            if (id_ordine > 0)
            {
                ...
                Populate.Populate_combobox_sedi(new ComboBox[] { FieldOrdOggSede }, idcl);
        ...
        private void DataGridViewOrd_CellDoubleClick(object sender, DataGridViewCellEventArgs e)
        {
            ...
            if (dgv.SelectedRows.Count == 1)
            {
                ...
                foreach (DataGridViewRow row in dgv.SelectedRows)
                {
                    ...
                    {
                        Populate.Populate_combobox_ordini_crea_offerta(ComboBoxOrdOfferta, cliente_id, sede_id, transformed: false, codice: ID_offerta);
                    }
        ...
        private void BtCreaOrdineOgg_Click(object sender, EventArgs e)
        {
            ...
            if (esito.Success)
            {
                if (UserSettings.Settings.Calendario.AggiornaCalendario == true)
                {
                    Outlook.Application OlApp = new();
                    Outlook.Folder personalCalendar = CalendarInteropOutlook.FindCalendar(OlApp, UserSettings.Settings.Calendario.NomeCalendario);
                ...
                try
                {
                    ...
                    using (SQLiteCommand cmd_up = new(commandText, ProgramParameters.connection, transaction))
                    {
                        try
                        {
                            ...
                            if (UserSettings.Settings.Calendario.AggiornaCalendario)
                            {
                                string ordinecode = "";
                                ...
                                if (!String.IsNullOrEmpty(ordinecode) && CalendarInteropOutlook.FindAppointment(personalCalendar, ordinecode))
                                {
                                    dialogResult = OnTopMessage.Question("Vuoi eliminare l'evento associato all'ordine?", "Eliminazione Evento Ordine Calendario");
                                    if (dialogResult == DialogResult.Yes)
                                    {
                                        CalendarInteropOutlook.RemoveAppointment(personalCalendar, ordinecode);
                                    }
                                }
            ...
            using (SQLiteCommand cmd = new(commandText, ProgramParameters.connection))
            {
                try
                {
                    ...
                    if (UserSettings.Settings.Calendario.AggiornaCalendario)
                    {
                        ...
                        Outlook.Folder personalCalendar = CalendarInteropOutlook.FindCalendar(OlApp, UserSettings.Settings.Calendario.NomeCalendario);
                        if (CalendarInteropOutlook.FindAppointment(personalCalendar, oldRef))
                        {
                            bool removed = false;
                            if (oldStato != stato_ordine && stato_ordine == 1)
                            {
                                ...
                                if (res != DialogResult.OK)
                                {
                                    CalendarInteropOutlook.RemoveAppointment(personalCalendar, oldRef);
                                    removed = true;
                                }
                            }
                            if (removed == false)
                            {
                                if (DateTime.Compare(oldETA, dataETAOrdValue.DateValue) == 0 && (oldPrezF != prezzo_finaleV.DecimalValue || oldRef != n_ordine))
                                {
                                    ...
                                    {
                                        CalendarInteropOutlook.UpdateCalendar(personalCalendar, oldRef, n_ordine, id_ordine, dataETAOrdValue.DateValue, false);
                                    }
                                }
                                else if (DateTime.Compare(oldETA, dataETAOrdValue.DateValue) != 0)
                                {
                                    ...
                                    if (res != DialogResult.Yes)
                                    {
                                        CalendarInteropOutlook.UpdateCalendar(personalCalendar, oldRef, n_ordine, id_ordine, dataETAOrdValue.DateValue);
                                    }
                                }
                            }
                        }
        ...
        private void ComboSelOrdCl_SelectedIndexChanged(object sender, EventArgs e)
        {
            ...
            if (idcl > 0)
            {
                Populate.Populate_combobox_sedi(new ComboBox[] { ComboSelOrdSede }, idcl);
        ...
        private void DataGridViewOrd_Filter_NumOrdine_TextChanged(object sender, EventArgs e)
        {
            TextBox inputBx = (TextBox)sender;
        ...
        private void FieldOrdOggMach_SelectedIndexChanged(object sender, EventArgs e)
        {
            ...
            Populate_combobox_ricambi_ordine(new ComboBox[] { FieldOrdOggPezzo }, id_mach);
        }
                    ...
                    if (!String.IsNullOrEmpty(filePath))
                    {
                        ...
                        using (var document = UglyToad.PdfPig.PdfDocument.Open(filePath))
                        {
                            ...
                            if (wordsCollection.Count() > 0)
                            {
                                ...
                                lang = Order.IdentifyLanguage(words);
                            }
                            ...
                            if (lang == "")
                            {
                                ...
                                if (isPDFImages == true)
                                {
                                    lang = ImportPDF.Helper.GetImageLanguage();

                                    ImageSavePath = ProgramParameters.exeFolderPath + System.IO.Path.AltDirectorySeparatorChar + System.IO.Path.GetFileNameWithoutExtension(filePath) + System.IO.Path.AltDirectorySeparatorChar;
                                    Directory.CreateDirectory(ImageSavePath);
                                    OCR.ExtarctImagesFromPDF(filePath, ImageSavePath);
                                    words = OCR.ExtractWordFromImage(ImageSavePath, lang, 1);
                                    words = words.OrderBy(a => a.X).ThenBy(a => a.Y).ToList();
                                }
                                else
                                {
                            ...
                            if (!isPDFImages)
                            {
                                ...
                                lines = OCR.ExtractLineFromImage(ImageSavePath, lang);

                            orderInfo = Order.ExtractHeader(words, lines, lang, isPDFImages).ToDictionary(kvp => kvp.Key, kvp => kvp.Value); ;
                        }
                        ...
                        for (int i = 0; i < c; i++)
                        {
                            ...
                            if (code.Success)
                            {
                                Dictionary<string, Dictionary<string, string>> findStrField = Helper.GetDictionarySDSS("OrdineItem");
                                ...
                                if (!String.IsNullOrEmpty(itemInfo["eta"]))
                                    itemInfo["eta"] = Order.Helper.ReturnEtaImportOrder(itemInfo["eta"]);
                        ...
                        OrderManager.Class.Offerte.Answer checkOfferta = OrderManager.Class.Offerte.GestioneOfferte.GetIfTransformed(orderInfo["numeroOff"].Trim());
        ...
        private async void CreaEventoCalendario_Click(object sender, EventArgs e)
        {
            UpdateFields("VS", "E", false);

            string nordine = VisOrdNumero.Text;
            string opde = VisOrdETA.Text;
            string OrdineId = VisOrdId.Text;
            ...
            bool? online = HelperProvider.IsEventGraph();
            bool EventExists = false;
            Outlook.Folder personalCalendar = null;
            GraphServiceClient graphClient = null;

            if (online == false)
            {
                Outlook.Application OlApp = new();
                personalCalendar = CalendarInteropOutlook.FindCalendar(OlApp, UserSettings.Settings.Calendario.NomeCalendario);
                EventExists = CalendarInteropOutlook.FindAppointment(personalCalendar, nordine);
            }
            else if (online == true)
            {
                string clientid = Properties.Settings.Default.ClientId;
                string scope = Properties.Settings.Default.Scopes;

                var authProvider = new GraphLogin(clientid, scope.Split(";"));
                graphClient = await authProvider.Authorize();
            ...
            if (!EventExists)
            {
                ...
                {
                    dateAppoint = CalendarManager.HelperData.CalendarDate_Create(dataETAOrdValue.DateValue);

                    if (dateAppoint == DateTime.MinValue)
                    {
                        UpdateFields("VS", "E", true);
                        return;
                    }

                    dateAppoint = CalendarManager.HelperData.CalendarDate_Check(dateAppoint, dataETAOrdValue.DateValue);
                }

                string body = CalendarManager.HelperEvent.CreateAppointmentBody(Convert.ToInt64(VisOrdId.Text.Trim()));

                if (online == false)
                    CalendarInteropOutlook.AddAppointment(personalCalendar, nordine, body, dateAppoint);
                else
                {

                    (bool result, string calendarSubgroup, string calendarId) = await CalendarGraph.FindCalendar(graphClient, UserSettings.Settings.Calendario.NomeCalendario, UserSettings.Settings.Calendario.NomeCalendarGroup);
                    if (result)
                    {
                        string uid = await CalendarGraph.AddAppointment(graphClient, calendarId, nordine, body, dateAppoint);
                        //update uid order
                    }
        ...
        private void RimuoviEventoCalendario_Click(object sender, EventArgs e)
        {
            ...
            if (CalendarInteropOutlook.FindAppointment(personalCalendar, nordine))
            {
                CalendarInteropOutlook.RemoveAppointment(personalCalendar, nordine);
            }
            else
            {
        ...
        private void AggiornaEventoCalendario_Click(object sender, EventArgs e)
        {
            ...
            if (CalendarInteropOutlook.FindAppointment(personalCalendar, oldRef))
            {
                CalendarInteropOutlook.UpdateCalendar(personalCalendar, oldRef, newRef, id_ordine, estDate, false);
            }
            else
            {
                OnTopMessage.Alert("Evento non presente.");
        ...
        private void AggiornaEventoDataCalendario_Click(object sender, EventArgs e)
        {
            ...
            CalendarInteropOutlook.AggiornaDataCalendario(personalCalendar, newRef);
        ...
        private void DuplicatiEventoCalendario_Click(object sender, EventArgs e)
        {
            ...
            CalendarInteropOutlook.FindCalendarDuplicate(personalCalendar, newRef);
        ...
        private void SettingSalva_Click(object sender, EventArgs e)
        {
            ...
            string nomeCalGroup = Convert.ToString(settingCalendarioGruppoNome.Text.Trim());
            ...
            bool isSameCalendar = UserSettings.Settings.Calendario.NomeCalendario != nomeCal;
            bool isSameGroup = UserSettings.Settings.Calendario.NomeCalendarGroup != nomeCalGroup;
            ...
            if (HelperProvider.IsEventGraph() == true)
            {

            }
            else if (HelperProvider.IsEventGraph() == false)
            {
            ...
            if (isSameGroup)
            {
                DialogResult dialogResult = OnTopMessage.Question("Stai per cambiare nome al gruppo del calendario, il software proverà a creare il calenmdario nel nuovo grupopo e spostare gli eventi pianificati da oggi in avanti nel nuovo calendario. In caso di errori, gli eventi rimanenti dovranno essere modificati manualmente. Continuare?", "Cambio Nome Gruppo Calendario");
                if (dialogResult == DialogResult.Yes)
                {
                    Outlook.Application OlApp = new();
                    if (CalendarInteropOutlook.MoveAppointment(OlApp, UserSettings.Settings.Calendario.NomeCalendario, nomeCal) == false)
                    {
            ...
            UserSettings.Settings.Calendario.AggiornaCalendario = upCalendar;
        ...
        //POPULTAE FUNCTIONS
        private static void Populate_combobox_machine(ComboBox[] nome_ctr, long idcl = 0, long idsede = 0, int deleted = 0)
        {
        ...
        private void Populate_combobox_ricambi_ordine(ComboBox[] nome_ctr, long idmc = 0)
        {
            var dataSource = new List<ComboBoxList>
            {
                new ComboBoxList() { Name = "", Value = -1 }
        ...
        private static void Populate_combobox_fornitore(ComboBox[] nome_ctr, int deleted = 0)
        {
            var dataSource = new List<ComboBoxList>
            {
                new ComboBoxList() { Name = "", Value = -1 }
        ...
        private static void Populate_combobox_offerte_crea(ComboBox[] nome_ctr, long idcl = 0, long idsd = 0)
        {
            string queryExtra = "";
        ...
        private static void Populate_combobox_ordini(ComboBox nome_ctr, long idcl = 0)
        {
            var dataSource = new List<ComboBoxList>
            {
                new ComboBoxList() { Name = "", Value = -1 }
        ...
        private static void ClearDataGridView(DataGridView nome_ctr)
        {
            nome_ctr.DataSource = null;
            nome_ctr.Rows.Clear();
        }

        internal static object ReturnStato(object stat)
        {
        ...
        private void UpdateClienti(int page = 0)
        {
            ...
            Populate.Populate_combobox_clienti(nomi_ctr);
            ...
            Populate.Populate_combobox_clienti(nomi_ctr, 1);
        ...
        private void UpdateClientiSedi(int page = 0)
        {
            ...
            Populate.Populate_combobox_dummy(nomi_ctrs: nomi_ctr);
        ...
        private void UpdatePRef(int page = 0)
        {
            ...
            foreach (KeyValuePair<ComboBox, ComboBox> ctr in nome_ctr)
            {
                if (ctr.Value.DataSource != null)
                {
                    ...
                    if (curItemValue > 0)
                    {
                        Populate.Populate_combobox_pref(ctr.Key, curItemValue);
                    }
                    else
                    {
                        Populate.Populate_combobox_dummy(ctr.Key);
                    }
                }
                else
                {
        ...
        private void UpdateOrdiniStato()
        {
            ...
            Populate.Populate_combobox_statoOrdini(nomi_ctr);
        }
        ...
        {
            settingCalendarioNome.Text = UserSettings.Settings.Calendario.NomeCalendario;
            settingCalendarioGruppoNome.Text = UserSettings.Settings.Calendario.NomeCalendarGroup;
            settingCalendarioDestinatari.Text = UserSettings.Settings.Calendario.Destinatari;
            settingCalendarioUpdate.Checked = UserSettings.Settings.Calendario.AggiornaCalendario;
        }
        ...
        private void Github_LinkClicked(object sender, LinkLabelLinkClickedEventArgs e)
        {
            ...
            switch (name)
            {
                ...
                case "Itext7":
                    ...
                    break;
                case "Ghostscript_site":
                    System.Diagnostics.Process.Start("https://artifex.com/licensing/agpl/");
                    System.Diagnostics.Process.Start("https://www.gnu.org/licenses/agpl-3.0.en.html");
                    break;
                default:
