...
using UserSettings = OrderManager.Class.UserSettings;

[assembly: Guid("4D443741-BB49-46B6-A978-117FC5801854")]
...
namespace OrderManager
{
    static class Program
    {
        ...
        static readonly string db_check_file = ProgramParameters.exeFolderPath + ProgramParameters.db_folder_path + ProgramParameters.db_file_name;
        ...
        private static void Main()
        {
            ...
            try
            {
                SQLiteConnection connection = new(@"Data Source = C:\Users\Luca\Documents\GitHub\OrderManager\bin\Release\net6.0-windows10.0.22621.0\win-x64\db\ManagerOrdini.db ;cache=shared; synchronous  = NORMAL ;  foreign_keys  = 1;  journal_mode=WAL; temp_store = memory;  mmap_size = 30000000000; ");
            }
            ...
            SupportClasses.DbCallResult versione = ReturnDbVersione();
        ...
        private static void CheckSingleProgramInstance()
        {
        ...
        private static void ValidateDB()
        {
            if (File.Exists(db_check_file) == false)
            {
                ...
                {
                    RunSqlScriptFile(Path.Combine(ProgramParameters.exeFolderPath, ProgramParameters.db_folder_path, @"\tables\tables.sql"), ProgramParameters.connectionStringAdmin);
                }
                else if (dialogResult == DialogResult.No)
                {
                    ...
                    if (dialogResult == DialogResult.Yes)
                    {
                        using (OpenFileDialog openFileDialog = new())
                        {
                            ...
                            {
                                File.Copy(openFileDialog.FileName, ProgramParameters.exeFolderPath + ProgramParameters.db_folder_path + ProgramParameters.db_file_name);
        ...
        private static SupportClasses.DbCallResult ReturnDbVersione()
        {
            SupportClasses.DbCallResult answer = new();
        ...
        private static void CheckSetting()
        {
            ...
            string setting_path = ProgramParameters.settingFile;
            if (!File.Exists(setting_path))
            {
                UserSettings UserSettings = new();

                UserSettings.Settings.Calendario.NomeCalendario = GetCalendarName();
                UserSettings.Settings.Calendario.NomeCalendarGroup = GetCalendarGroupName();
                ...
                if (dialogResult == DialogResult.Yes)
                {
                    UserSettings.Settings.Calendario.AggiornaCalendario = true;

                    //settings["calendario"].Add("aggiornaCalendario", "true");
                }
                else
                {
                    UserSettings.Settings.Calendario.AggiornaCalendario = false;
        ...
        private static string GetCalendarGroupName()
        {
            string input = null;

            while (input == null)
                input = Interaction.InputBox("Impostare un nome per il Gruppo del Calendario in cui verra aggiunto il calendario contenente i rememnder per gli ordini." + Environment.NewLine + Environment.NewLine + "Se lasciato vuoto, verrà usato il gruppo di default di Outlook", "Nome Gruppo Calendario Eventi", "")
                    .Trim();

            return input;
        }

        private static void CheckDbUpdate(SupportClasses.DbCallResult versione)
        {
                ...
                foreach (FileInfo file in Files)
                {
                    ...
                    if (decimal.TryParse(fnames_ver[index_str].Replace('.', ','), out decimal dec))
                    {
                        if (versione.DecimalValue < dec)
                        {
                            ...
                            bool success = RunSqlScriptFile(Path.Combine(ProgramParameters.exeFolderPath, db_update_folder, file.Name), ProgramParameters.connectionStringAdmin);
        ...
        private static void BkBackup(bool automata = false)
        {
            using (FolderBrowserDialog db_backup_path = new())
            {
                ...
                if (automata == true || dialogreturn == DialogResult.OK)
                {
                    ...
                    {
                        bk_fileName = ProgramParameters.exeFolderPath + ProgramParameters.db_folder_path + nameTempDbRetsore;
                    }
        ...
        {
            string filePath = ProgramParameters.exeFolderPath + ProgramParameters.db_folder_path + nameTempDbRetsore;
        ...
        {
            DirectoryInfo d = new(ProgramParameters.exeFolderPath + ProgramParameters.db_folder_path);
